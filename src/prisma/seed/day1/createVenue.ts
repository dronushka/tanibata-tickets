...
export default async function createVenue () {
    let mailMessage = '\
        ...
        Билет даёт пропуск на один день фестиваля <strong>(это билет на первый день - 15 июля)</strong>.</p> \
        ...
        <p>Если вы захотите остаться после программы косплей-шоу 1-го дня фестиваля на концерт, билеты на него можно будет приобрести в ОДНТ до начала концерта в 21:00.<br> \
        Билеты на танибатаходики будут в продаже до самого отплытия - разумеется, <em>при их наличии</em>.</p> \
    ...
    '
    const ticketTemplatePDF = "day1show.pdf"
    const premiumTicketTemplatePDF = "day1showPremium.pdf"

    fs.copyFileSync(`prisma/seed/day1/${ticketTemplatePDF}`, `${process.env.LOCAL_FILE_STORAGE}/${ticketTemplatePDF}`)
    fs.copyFileSync(`prisma/seed/day1/${premiumTicketTemplatePDF}`, `${process.env.LOCAL_FILE_STORAGE}/${premiumTicketTemplatePDF}`)
