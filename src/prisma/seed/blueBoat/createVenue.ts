...
export default async function createVenue () {
    let mailMessage = '\
        ...
        <p><strong>Как пройти на Голубой Танибатаход?</strong></p>\
        ...
        Билет даёт пропуск на Фестивальные Автобусы и <strong>Голубой Танибатаход</strong>.</p>\
        ...
        <p><strong>В связи с требованиями противотерристической безопасности мы настоятельно рекомендуем брать с собой оригинал удостоверения личности!</strong></p>\
    '
    ...
    const ticketTemplatePDF = "blueBoat.pdf"
    const premiumTicketTemplatePDF = "blueBoatPremium.pdf"

    fs.copyFileSync(`prisma/seed/blueBoat/${ticketTemplatePDF}`, `${process.env.LOCAL_FILE_STORAGE}/${ticketTemplatePDF}`)
    fs.copyFileSync(`prisma/seed/blueBoat/${premiumTicketTemplatePDF}`, `${process.env.LOCAL_FILE_STORAGE}/${premiumTicketTemplatePDF}`)
