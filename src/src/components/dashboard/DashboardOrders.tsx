...
import { File as DBFile, Order, OrderStatus, Ticket, User, Venue } from "@prisma/client"
...
export default function DashboardOrders({
    ...
    const updatePageParams = ({
        ...
        // console.log(params)
        if (newFilter || newStatus) params.page = 1
        else if (newPage) params.page = newPage

        if (newFilter !== undefined) params.filter = newFilter

        if (newStatus !== undefined) params.status = newStatus

        if (params.page === pagination.page && params.filter === filter && params.status === status) return
    ...
    return (
        <Stack>
            <Paper p="sm" shadow="xs">
                <Stack>
                    <Group>
                        ...
                        />
                        <Button leftIcon={<IconSearch />} onClick={() => updatePageParams({ newFilter: searchFilter })}>
                            Найти
                        </Button>
                    </Group>
                    <OrdersStatusFilter
            ...
            {isPending && <Progress radius="xs" size="sm" value={100} animate />}

            {!isPending && (
                <>
                    <Stack>
                        {orders.map((order) => (
                            <Paper key={order.id} p="sm" shadow="xs">
                                    ...
                                    <Stack sx={{ alignItems: "center" }}>
                                        ...
                                        <Box sx={{ flexShrink: 1 }}>
                                            <Link href={"/dashboard/orders/" + order.id} passHref legacyBehavior>
                                                <Button component="a" variant="subtle" leftIcon={<IconEdit />}>
                                                    Изменить
                                                </Button>
                                            </Link>
                                        </Box>
