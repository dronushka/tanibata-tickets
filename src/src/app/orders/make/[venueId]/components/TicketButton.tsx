import { Box, Button, MantineColor, Popover, Stack, Sx, Text } from "@mantine/core"
...
export default function TicketButton({
            ...
            if (prev.has(ticket.id)) {
                ...
                return new Map(
                    [...newTickets.entries()].sort(([k, v], [k2, v2]) => {
                        return (
                            (v.rowNumber ? parseInt(v.rowNumber) : 0) - (v2.rowNumber ? parseInt(v2.rowNumber) : 0) ||
                            parseInt(v.number) - parseInt(v2.number)
                        )
                    })
                )
            }
    ...
    if (selected) colorCode = "#228be6" //"#daca55" //"#da7855"
    else if (ticket.priceRange?.color) colorCode = ticket.priceRange.color

    if (reserved || ticket.reserved) return <Box sx={{ 
        ...sx,
